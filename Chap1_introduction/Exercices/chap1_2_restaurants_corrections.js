...
db.restaurants
  .find(
    ...
    { name: 1, _id: 0, "grades.score": 1, "address.coord": 1 }
  )
  .sort({
    name: 1,
  });
...
db.restaurants
  .find(
    ...
    { name: 1, _id: 0, "grades.grade": 1, "grades.score": 1 }
  )
  .sort({ name: -1 });
...
db.restaurants.find(
  {
    $and: [{ name: /coffee/i }, { borough: "Bronx" }],
  },
  { _id: 0, name: 1, borough: 1 }
);
...
db.restaurants
  ...
  .forEach((doc) => {
    ...
    print();
    print(
      `name : ${name.toUpperCase()} coordinate : ${address.coord.join(" ")}`
    );
  });
